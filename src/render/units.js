...
/** Infantry: crossed rifles, which on a map is a box with an X in it. */
...
/** An aeroplane from above: fuselage, swept wings, tailplane. */
function aircraft(ctx, x, y, s) {
  ctx.beginPath();
  ctx.moveTo(x, y - s * 0.6);
  ctx.lineTo(x + s * 0.16, y - s * 0.1);
  ctx.lineTo(x + s * 0.75, y + s * 0.18);
  ctx.lineTo(x + s * 0.75, y + s * 0.32);
  ctx.lineTo(x + s * 0.14, y + s * 0.24);
...
/** A warship in profile: hull, bridge, one funnel. */
function warship(ctx, x, y, s) {
  ctx.beginPath();
  ...
  ctx.stroke();
  ctx.beginPath();
  ctx.rect(x - s * 0.2, y - s * 0.28, s * 0.34, s * 0.38);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
...
function carrier(ctx, x, y, s) {
  ...
  ctx.stroke();
  ctx.beginPath();
  ctx.rect(x + s * 0.1, y - s * 0.24, s * 0.2, s * 0.29);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(x - s * 0.55, y - s * 0.14);
  ctx.lineTo(x - s * 0.15, y - s * 0.14);
  ctx.moveTo(x - s * 0.35, y - s * 0.34);
...
function submarine(ctx, x, y, s) {
  ctx.beginPath();
  ctx.ellipse(x, y + s * 0.22, s * 0.8, s * 0.2, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ctx.rect(x - s * 0.14, y - s * 0.16, s * 0.3, s * 0.3);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
...
/** Icon size and how many of them fit across a cell at this zoom. */
function sizing(pixelsPerCell) {
  const size = Math.max(8, Math.min(28, pixelsPerCell * 0.26));
  const most = Math.max(1, Math.min(3, Math.floor(pixelsPerCell / 15)));
  return { size, most, labels: size >= 15 };
}

/** Draw one row of symbols, centred, with counts under them if there is room. */
function drawRow(ctx, entries, x, y, size, labels) {
  const gap = size * 1.5;
  const left = x - ((entries.length - 1) * gap) / 2;
  entries.forEach((entry, k) => {
    const cx = left + k * gap;
    ctx.lineWidth = Math.max(1, size * 0.07);
    ctx.strokeStyle = 'rgba(6, 10, 16, 0.9)';
    ctx.fillStyle = entry.color;
    entry.draw(ctx, cx, y, size);
    if (!labels) return;
    ctx.font = `600 ${Math.round(size * 0.62)}px 'Inter', 'Segoe UI', system-ui, sans-serif`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    const text = entry.count >= 10000 ? `${Math.round(entry.count / 1000)}k` : String(entry.count);
    ctx.lineWidth = 3;
    ctx.strokeStyle = 'rgba(0, 0, 0, 0.85)';
    ctx.strokeText(text, cx, y + size * 0.62);
    ctx.fillStyle = '#eef4fb';
    ctx.fillText(text, cx, y + size * 0.62);
  });
}
...
export function drawUnitIcons(ctx, world, camera, width, height, viewer) {
  ...
  const { size, most, labels } = sizing(pixelsPerCell);
  ...
  for (const cell of cells) {
    ...
    if (!canSeeForces(viewer, nation)) continue;

    // The heaviest arms on the cell, by what they contribute to its strength:
    // three hundred riflemen and one bomber should both get a symbol.
    ...
    drawRow(ctx, present, out.x, out.y, size, labels);
  }
...
export function drawFleetIcons(ctx, station, x, y, pixelsPerCell) {
  ...
  const { size, most, labels } = sizing(pixelsPerCell);
  // In the order the types are listed, which is heaviest first: one battleship
  // says more about what is moored here than twenty destroyers do.
  ...
  drawRow(ctx, present, x, y, size, labels);
  return true;
}
